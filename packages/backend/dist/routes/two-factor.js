...
const twoFactorService = __importStar(require("../services/two-factor.service.js"));
const auth_js_1 = require("../middlewares/auth.js");
const async_handler_js_1 = require("../middlewares/async-handler.js");
const errors_js_1 = require("../lib/errors.js");
const router = (0, express_1.Router)();
router.use(auth_js_1.authMiddleware);
router.post('/setup', (0, async_handler_js_1.asyncHandler)(async (req, res) => {
    const result = await twoFactorService.setup2FA(req.user.sub);
    res.json(result);
}));
router.post('/enable', (0, async_handler_js_1.asyncHandler)(async (req, res) => {
    const { token } = req.body;
    if (!token)
        throw new errors_js_1.ValidationError('Token é obrigatório');
    await twoFactorService.enable2FA(req.user.sub, token);
    res.json({ message: '2FA ativado com sucesso' });
}));
router.post('/disable', (0, async_handler_js_1.asyncHandler)(async (req, res) => {
    const { token } = req.body;
    if (!token)
        throw new errors_js_1.ValidationError('Token é obrigatório');
