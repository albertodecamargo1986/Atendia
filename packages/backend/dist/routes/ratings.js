...
router.get('/summary', (0, async_handler_js_1.asyncHandler)(async (req, res) => {
    const result = await ratingService.getRatingsSummary(req.user.tenantId);
    res.json(result);
}));
router.post('/:ticketId/rate', (0, async_handler_js_1.asyncHandler)(async (req, res) => {
    const { score, comment } = req.body;
    if (!score || score < 1 || score > 5)
        throw new errors_js_1.ValidationError('Nota deve ser entre 1 e 5');
    const result = await ratingService.rateTicket(req.params.ticketId, req.user.tenantId, score, comment);
    res.status(201).json(result);
}));
router.get('/:ticketId/rating', (0, async_handler_js_1.asyncHandler)(async (req, res) => {
